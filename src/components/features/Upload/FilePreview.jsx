...
const FilePreview = ({ file, previewUrl, onReset }) => (
    <div className="bg-slate-800/50 p-4 rounded-2xl border border-slate-700 flex flex-col h-full">
        <h2 className="text-xl font-bold text-slate-100 mb-4 text-center">Preview</h2>
        <div className="flex-1 flex flex-col items-center justify-center text-center bg-slate-900/50 rounded-lg p-4 min-h-[200px]">
            {previewUrl && file?.type.startsWith('image/') ? (
                ...
                <div className="space-y-4">
                    ...
                    <p className="text-sm text-slate-500">A visual preview is not available for this file type.</p>
                </div>
            )}
        </div>
        <div className="mt-4 pt-4 border-t border-slate-700 flex items-center justify-between">
            <span className="text-sm font-medium text-slate-400 truncate pr-4">{file.name}</span>
            <button onClick={onReset} className="text-slate-500 hover:text-red-400 transition-colors flex-shrink-0"><ResetIcon /></button>
        </div>
    </div>
);
